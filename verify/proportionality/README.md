...
| `render.py` | stamps the £-derived action into the body per institution (reuses `platform/risk/enforce.py`, fetched by [`../../clone-estate.sh`](../../clone-estate.sh) into `../../.estate-clone/platform/` — platform is a real, separate GitHub repo post-split, not a sibling directory); `--check` guards drift |
...
source of the £ maths — no risk engine is reimplemented here. (Post-split:
that's the real `policy-as-versioned-platform` repo, fetched locally by
`../../clone-estate.sh`.)
