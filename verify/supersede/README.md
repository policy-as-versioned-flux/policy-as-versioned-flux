...
line's own amount. Which composer wrote the evidence is read off the adopter's own compiler pin
at that commit, `.github/platform-tools-pin.yaml`, and off its policy pin
`gitops/platform/platform-pin.yaml` only where no compiler pin exists (ticket 110: since
2026-09-10 the adopters compose with a compiler pinned apart from the policy they accept). A
pinned composer that carries no supersede rule is a could-not-look naming the tag, because no
...
`supersede.py selfcheck` plants 30 grades and proves each bites. Ticket 128 added five: a
line whose target the adopter's pinned checkout could not read says `readable: false`, names the
newest signed tag ahead and carries no `published_at`; and a missing line under a composer that
predates ticket 128 is still a FAIL, with that reason named. What it refuses to grade
