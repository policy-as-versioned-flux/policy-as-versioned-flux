...
- **The grade change depends on the facts observed when the merge is measured** (corrected
  2026-09-10 during integration review). Point 6 stops a could-not-look from softening an observed
  FALSE. Each adopter whose newest sample carries such a FALSE may move from SKIP to FAIL;
  `verify/e2e/verify-e2e-step4-flux-reconciles-cage.sh` follows driftwood when its sample does.
  Determine the actual changed rows from the two recorded grade tables on the day, and acknowledge
  only the observed fall with its real run number. There is no fixed three-or-four prediction:
  the earlier four-row measurement described run 225, while ticket 107's subsequent samples
  changed the population before integration. This correction changes the instrument's reporting,
  not the underlying estate state.
