#!/usr/bin/env bash
# Idempotent bring-up of the ludlow institution:
#   KinD cluster  ->  Flux  ->  real GitHub git source  ->  reconcile healthy
# Same shape as driftwood's scripts/up.sh (the provenance base) — ludlow inherits
...
# policy-as-versioned-ludlow/ludlow and policy-as-versioned-nist/nist repos on
# GitHub (see gitops/flux-system/gotk-sync*.yaml), so step 3 below fails fast
...
if kind get clusters 2>/dev/null | grep -qx "$CLUSTER"; then
  ...
  kind create cluster --name "$CLUSTER" --config "${HERE}/kind/ludlow.yaml" --wait 120s
fi
...
say "applying ludlow + nist GitRepository/Kustomization sources"
...
flux reconcile source git ludlow --context "$CTX" --timeout 30s
flux reconcile source git nist --context "$CTX" --timeout 30s
flux reconcile kustomization ludlow --with-source --context "$CTX" --timeout 60s

say "done. status:"
flux get sources git ludlow --context "$CTX"
flux get sources git nist --context "$CTX"
flux get kustomizations ludlow --context "$CTX"
echo
say "run  estate/ludlow/verify-reconcile.sh  to assert the beat"
