#!/usr/bin/env bash
# Idempotent bring-up of the driftwood institution:
...
# policy-as-versioned-driftwood/driftwood and policy-as-versioned-nist/nist repos
...
say "applying driftwood + nist GitRepository/Kustomization sources"
...
flux reconcile source git driftwood --context "$CTX" --timeout 30s
flux reconcile source git nist --context "$CTX" --timeout 30s
flux reconcile kustomization driftwood --with-source --context "$CTX" --timeout 60s
