...
> **SUPERSEDED 2026-08-29 (eco-system ticket 47).** This hand-authored deck is
> kept as the record and is not rewritten. It was written 2026-07-31, before the
> cage pivot and before composition, computed semver and the twin existed, and it
> still carries wording those decisions reversed. The deck that is shown is
> [`deck.md`](deck.md), which is **generated** by
> [`build_deck.py`](build_deck.py) from the truth surface's own captures under
> `talk/captures/`, with its prose in [`narration.json`](narration.json) and its
> checks in [`verify-demo.sh`](verify-demo.sh). It was renamed, not edited: the
> generated deck needs the `deck.md` path that everything downstream reads.
...
verify-*.sh that exits 0 (talk/verify-all.sh proves the whole set).
Slides tagged [NARRATED] are real + grounded but gestured, not stood up.
Full run order, reset, and audience-modular foregrounding: talk/RUNBOOK.md.
...
```sh
python3 .estate-clone/platform/fair/fair.py summary \
  .estate-clone/platform/fair/scenarios/driftwood-cart-pii.json
```
...
```sh
.estate-clone/platform/distribution/verify-coexistence.sh   # two versions admit side-by-side
.estate-clone/platform/distribution/verify-orphan-guard.sh  # a version not in the array cannot run
.estate-clone/platform/distribution/verify-retirement.sh    # deleting one array element prunes it
```
...
```sh
.estate-clone/platform/shift-left/verify-shift-left.sh
```
...
```sh
.estate-clone/platform/policy/verify-conditional.sh   # run root IF attested AND hardened
```
...
```sh
.estate-clone/platform/graded/verify-graded.sh   # tiers->dials deterministic; £ picks the tier
.estate-clone/platform/tcor/verify-tcor.sh        # the board line: Total Cost of Risk, and it MOVES
```
...
```sh
.estate-clone/platform/feeds/verify-feeds.sh      # a feed bump arrives as a reviewable diff that moves the £
.estate-clone/platform/wardley/verify-wardley.sh  # commoditisation MOVEMENT -> a forward signal, re-tune early
.estate-clone/platform/wargamer/verify-wargamer.sh # opens a SIGNED PR, never merges; carries the version gate
```
...
```sh
.estate-clone/platform/honesty/verify-honesty.sh
```
...
```sh
.estate-clone/platform/identity/verify-identity.sh              # SPIRE is Istio's CA, mTLS STRICT
.estate-clone/platform/posture/verify-posture-projection.sh     # posture/vN in the SVID path; forging refused
.estate-clone/tuppence/reset/verify-reach-secrets.sh            # out-of-currency caller loses reach AND its secret
```
...
```sh
.estate-clone/platform/access/verify-access.sh        # Pomerium OIDC + WebAuthn; device SVID on the same root
.estate-clone/platform/break-glass/verify-break-glass.sh # a risky op demands step-up — by the £
.estate-clone/platform/eud/verify-eud.sh              # tpm_devid device SVID, same acme.internal root
```
...
```sh
talk/verify-all.sh          # 25 offline beats — the whole deck's honesty gate
talk/verify-all.sh --live   # + the 3 institution reconcile beats (needs cluster)
```

**Run it live. Nothing here is rounded up to 100%.** The parts that work, work
live; anything narrated is named and scoped.

- Idempotent, resettable, audience-modular bring-up (network needed to fetch
  the six real units — no more offline guarantee, see `talk/RUNBOOK.md`):
  **`talk/RUNBOOK.md`**
- Re-foreground the room in one command:
  `talk/up.sh foreground {driftwood|tuppence|ludlow}`
