...
- [x] The generator enumerates each predicate expression across satisfied, violated and absent.
- [x] The version-pin axis covers inside and outside the array.
- [x] The tier axis covers absent, `baseline`, `restricted` and `quarantine`.
- [x] The axes combine pairwise, never fully.
- [x] The generator runs against both subjects and unions the result.
- [x] The document reports the old count, the new count and the union count.
- [x] Each entry is one plain pod file, with no band and no residual.
- [x] The manifest carries the checksum, the entry count and per-witness provenance.
- [x] Claim source lives in the manifest, never on the entry.
- [x] CI regenerates the spine and fails on any diff.
- [x] The generator carries its own version, and the document reports it as `generator_version`.
- [x] `wall_clock` is measured and published, and no check enforces it.
- [x] No size ceiling exists anywhere.

## Comments

Shipped in `platform` at `3386151` + `4c201f2` + `d0fd9ff` (cs-19).
