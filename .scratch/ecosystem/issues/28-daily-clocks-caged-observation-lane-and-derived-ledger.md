...
**Corrected 2026-09-04 (ticket 56, delegated under ADR-0025).** That done-line over-claimed. The check was in `talk/verify-all.sh` on 2026-08-29, and its fourth question -- "did each clock actually run inside its own period", the only one that asks whether the clocks WORK rather than whether they are declared correctly -- had never graded on a citable run and could not. The gate step carries no GitHub credential on purpose (`persist-credentials: false`, no `GH_TOKEN`; 84 verify scripts from eight other organisations run in that job), so `schedules.py` step 4 called `gh auth status`, failed, and emitted one SKIP per clock. Run 22 of 2026-09-03 shows `skip=18`. The TRUTH line of 2026-08-29 therefore recorded that the clocks were declared, timed and caged -- which was true -- and recorded nothing at all about whether any of them had ticked. On 2026-09-04, when the question was finally asked for real, five of thirteen clocks were red and had been for days. From 2026-09-04 `truth.yml` runs a separate `clocks` job that holds `actions: read` and hands the gate a file of facts; the fourth question grades on the citable run without a credential ever reaching a third-party script. ADR-0024 carries the dated note and the two smaller corrections that came with it (a cancelled run is no longer excused; "this clock opens no PR" is a PASS with a named limit, not a SKIP).
