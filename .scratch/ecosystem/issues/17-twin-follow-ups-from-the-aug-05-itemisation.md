...
- `.scratch/twin/build/65-the-flux-falsification-verdict.md`: Status now reads `CLOSED UNMEASURED
  ...
  (does not fire) and the checklist (7/9 stands; the two open items close as unmeasured). A dated
  bracket at the "blocking chain" paragraph and a closing section "Closed unmeasured —
  2026-09-03" carry the reasoning.
...
this ticket's and is untouched). The title's 211 is captures; distinct decisions are 207, and the
note says so. `REVERSALS.json`, `DECISION_AUDIT.json` and appendix C carry no status field, only
`verdict: reverse`, which already is the rejection in the reversal record; they are generated
evidence and were not hand-edited.
...
were opened by the assistant with `gh pr create` under the owner's account; the merge was
declined by Claude Code's permission classifier, not by `enact_guard.py`; and hub `d81f202` the
same day fixed the guard's bare-remote hole that had admitted the pushes. Nothing in `twin/`
opened them. The eco-system's real proposer is each adopter's `propose-tier.yml`, which first
...
than deleted. `Blocked by` on 66 reads `none — was 65, relaxed 2026-09-03` rather than a bare
`none`, so the relaxation is visible without a `git blame`.
