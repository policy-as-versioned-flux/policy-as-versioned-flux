...
1. **The served workloads run.** Each adopter moves its own `gotk-sync.yaml` pin to its newest signed tag by a reviewed pull request. On 2026-09-25 each still pinned `v1.0.0`, whose tree lists no lifted app. The lane reconciles `./gitops/apps`. A new fact 8, in its own pre-registered section of `drift/window.yaml`, records for each served workload whether it runs in its cage, in the API server's words. A reference copy of the same workload runs outside the governed namespace on the same node. "Does not fit the cage" is graded only when the reference copy runs and the caged copy does not. The runner size is a recorded ceiling. A separate section does not restart facts 6 and 7. A workload that does not fit its rung is a priced outcome, never a carve-out. Fact 8's reference copy is decided here. Ticket 152 found that under 5.0.0 a pod that claims a version, in a Namespace that declares no tier, lands on `isolated`, and chose for fact 7 an unclaimed pod in a Namespace that declares nothing. Fact 8's reference copy meets the same rule.
