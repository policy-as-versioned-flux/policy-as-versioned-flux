...
Map line: `- [96 — The citable line says whether the twin may write to the world](issues/96-the-citable-line-says-whether-the-twin-may-write.md) — every TRUTH line the gate emits now carries `enact=<mode>`, the mode `twin/enact_guard.py` was at while the run happened, so a reader of `talk/truth.log` can tell whether the run they cite had the enactment refusal on or off. It sits between `hub=` and `units=` because run, hub and enact say what the RUN was while units onward says what it MEASURED, and because `live=1` and `fixture=1` are conditional flags an unconditional field must not be mistaken for; that placement moved one anchored selfcheck pattern instead of four. The word comes from `enact_mode()` itself, never re-derived in bash, so the line cannot disagree with the guard it describes; only the last stdout line is read and shape-checked, so import-time noise cannot split the citable line; `enact=unknown` is written when the mode cannot be resolved, and the NEWEST recorded unknown is red (older ones are named, not failed, so the red has a finishing move). REPORTED, NEVER GRADED: no mode is a failure and nothing invents a place where an authorisation would be recorded — that is ticket 97 and ticket 87 item 3, both the owner's. A line predating the field parses as None, which no parser may read as a mode and none may require. Graded by `verify-all.sh --selfcheck` (a fixture gate at each of the three modes) and `verify-truth-line.sh` leg 6, which reports how many recorded lines name the mode as a live number rather than a disclosure that goes stale.`
