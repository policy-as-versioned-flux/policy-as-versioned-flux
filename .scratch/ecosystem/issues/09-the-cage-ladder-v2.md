...
Resolved 2026-08-28. The owner read every held round and wrote: "ive already read the recommendations and I can't find fault with a single one. Well done. Get everything ready for me to then to-spec". Under the map's process rule a bare agree does not ratify. Each item below is PROVISIONAL, with the recommendation's own reason as rationale, as tickets 04, 07 and 08 were. The five-per-day rule was overridden by the owner's instruction. No D1-D5 decision names this ticket directly. Cross-ticket amendments C1, C7, C8 and C9 apply and are recorded below.

1. The tier attaches to the governed Namespace. Each governed Namespace manifest carries `posture.acme.io/tier` next to `governed: "true"` on the same signed object. No new object exists. One tier per adopter cannot de-posture one workload. A pod-label declaration is what reversal 13 reversed. De-posture of one workload is a move into a Namespace at the tighter tier. A per-workload override waits until an adopter needs it. C7: `cage-tier` reads `namespaceObject` and clobbers the pod label. That is the only Namespace-to-pod render. Ticket 12's SVID tier segment reads the rendered label. There is no second MutatingPolicy.

2. The cage mutation is tighten-only. It never writes a security field looser than the pod declared. This lands in all three served copies of `cage-tier`. A cage is a floor on the workload, not a ceiling. The change is two mutations gated on `harden`. `cage_engine.py` must classify "writes false over true" as a loosening, or the change classifies as `none`.

3. The bottom rung is `isolated`. It is the quarantine dials plus no ingress and no egress, generated by `cage-netpol` per tier, plus first eviction. "Not functional" is then literally true and every pod still runs. `select_tier` returns `isolated` where it returned `deny`. `cage-netpol` gains per-tier NetworkPolicies with `Ingress`: baseline normal, quarantine DNS only, isolated none. Enabling `synchronize` costs a brief delete-then-regenerate gap on a tier move. That gap is named. C1: the ladder value space is `baseline, restricted, quarantine, isolated, infra`.

4. The floor is `overlay.floor: <tier>` on `party.yaml`, one per adopter. The selection policy clamps to `max(selected, floor)` in ladder order. A PR that lowers or removes the floor is priced as a delta on `prices[]`. It is never refused (re-grill 16). Per-Namespace floors wait until an adopter needs two. C9: the floor is a constraint on selection. It is not a default tier and not an appetite object. Ticket 15 Q4's rejection of an adopter default tier stands beside it.

5. Any party whose signed `party.yaml` carries the `platform` role may declare a Namespace at `infra`. A declaration from a party without the role renders to `isolated`. One signed identity, no allowlist file. C8: the platform's declaration is `posture.acme.io/tier: infra` on its own Namespace manifests for `kube-system`, `flux-system` and `kyverno`. No package or artefact carries a namespace list (ADR-0018 §1 holds). Ticket 12 Q5's "rendered from the pinned package" is amended to this. The platform `party.yaml` (ticket 04 A4) carries roles, size, appetite and publishes. Order: that declaration lands and the truth surface asserts it before the default for an unlabelled governed Namespace flips from `baseline` to `isolated`. Otherwise CoreDNS lands in `isolated` and the cluster stops.

Consequences:
- One superseding ADR, ADR-0022, replaces ADR-0014's CREATE deny, ADR-0015's Deny-to-issue, ADR-0016 §3 and ADR-0018 §4. Those four gain supersession banners. ADR-0018 §1 holds.
- The pod label becomes an output only. H8-03 closes.
- Ticket 21 builds the platform `party.yaml` and the `overlay.floor` key. Ticket 25 carries no change from this ticket.
- A new build ticket, "The cage ladder lands", builds items 1 to 5 with the ordering rule and the offline `kyverno test` values file.
- Revisit triggers: an adopter needs a per-workload override or a second floor (reopens Q1 and Q4, and ticket 05's Cedar edge); the `synchronize` gap is observed on a real tier move; a fourth served copy of `cage-tier` appears.
- Later rounds still held: the warn rung; de-posture keeping the claim (H2-12); `access.py` retirement and break-glass bands per org appetite (H8-09, H8-12); pricing a tier move against `prices[]`.

Graduated:
- The cage ladder lands (task).
- ADR-0022: everything is caged (task).
- The cage ladder, round 2 (grilling): warn rung, de-posture, break-glass bands, tier-move pricing.
