...
The 2026-09-02 review found that every dimension returns "fit as X, not fit as Y", and that the record never states X. No document says what the estate is for, who receives it, or by when. NORTH-STAR §4 calls itself "my proposal" and its ratification is one line. Twelve questions decide the verdict. Each is put with the trade first. My call follows, labelled as mine, so it can be ignored. Answer with a letter and a reason. A bare letter is recorded provisional, as the map's rule says.

## Notes

Charted by [REVIEW-2026-09-02.md](../REVIEW-2026-09-02.md) §7. The purpose analyst's full reconstruction, with the owner's dated words, is at [review-2026-09-02/evidence/purpose/](../review-2026-09-02/evidence/purpose/). Tickets 79, 84, 86 and 87 are blocked on this round. Ticket 63 waits on Q3.

## Held round (2026-09-02)

❓ **Q1** - **What is this for, who receives it, and by when?** The record supports four purposes: (a) a touring talk for principal engineers and leaders that proves the corrected 2022 thesis as running code; (b) a reference implementation ControlPlane lifts into client work; (c) a system a fourth organisation could adopt; (d) the argument itself as a written, checkable artefact. Today the estate is nearly fit for (a), fit for (d), not fit for (b) for want of a licence and onboarding, and not close for (c). Is there a date, a venue or a named recipient? The last dated audience statement is 2026-07-23.

➡️ My call: (a) primary, (d) secondary, (b) downstream at the cost of one LICENSE file. It is the only purpose every dated instruction supports without contradiction.

❓ **Q2** - **Is NORTH-STAR §4 your definition of done, or my build order?** §4 says "my proposal". If it is your definition of done, the estate is not fit: step 3 has never fired, step 5's clock has failed both firings, step 4 holds for one adopter of three. If it is a build order, the estate is fit as work in progress and about twenty findings become schedule items. Folded in: does step 2 count as done when driftwood PR #20 raised the pin and the price did not move?

➡️ My call: (b) build order, with a dated line in the file saying so. Your 2026-08-04 words describe a modelled ambition with a chosen slice.

❓ **Q3** - **Does "at least three coexisting versions with a retirement window" still bind?** The estate declares one line. Ticket 63 reaches two. The phrase "at least three" in the thesis transcription is the transcriber's; the talk gives no number; your Medium post returns 403 to automation and I could not read it. You wrote it. Options: (a) three binds, scoped as three declared lines and a priced supersede; (b) two is the bar, and CONTEXT.md is corrected; (c) cages superseded the rule, and the estate then has no promise to an adopter who is behind.

➡️ My call: (a), because the retirement window is the thesis's stated reason for coexistence, and nothing prices being behind today.

❓ **Q4** - **Is the £ a decision instrument or a balance-sheet quantity?** Every input that turns a severity into money is an editorial constant. Two of the five anchor fines are legally stale: Doorstep is £92,000, not £275,000; Clearview has never been collected. Correcting them moves driftwood's largest line by minus 63 percent and the insurer's premium by minus 44 percent. The insurer's premium implies a 2,179 percent loss ratio. Options: (a) an ordinal, auditable comparison instrument, said so on the artefact; (b) a quantity a CFO books and a carrier cedes against, which needs grounded frequency, an aggregate distribution and a defensible quote. Folded in: is `appetite.tolerance` one quantity or three, and is the insurer a real second opinion or an illustrative counterparty?

➡️ My call: (a), stated on the artefact, with the stale fines corrected and a litigation-status field added either way.

❓ **Q5** - **"There is no gate": your words, or my reading?** NORTH-STAR:31 says the bridge from your words to "a refusal is the bottom rung" is mine. Your blog names access control, data protection and key management as needing a locked door. The estate ships policy in none of the three; the one control that demonstrates the split lives only in the hub harness; two Deny policies ship anyway and three documents disagree about whether they may. Options: (a) cages all the way down, and the two Denys become the bottom rung; (b) the locked door survives for those three, and one real data-protection control is published as a versioned member.

➡️ My call: (b), narrowly. `verify/proportionality` already derives Audit versus Deny from each party's signed band and has no shipped subject.

❓ **Q6** - **Does "a human merges" still bind, and who is the second human?** Zero rulesets and zero branch protection on nine repos. Every merged PR has author equal to merger. You said author and merger must differ for now and that AI disposal is the end state. The AI's only restraint is a mode file the AI writes. Options: (a) it binds: protect main and release/*.x with a required review from a different identity, and say who; (b) stepping stone: record the single-operator state in §6 and stop presenting principle 5 as enforced.

➡️ My call: (a) for the demonstration, (b) recorded as the end state. Step 3 is where an audience decides whether this is governance or automation.

❓ **Q7** - **Are driftwood, tuppence and ludlow props, or plausible firms?** Your instruction was fictitious and uncut. Today all three sit at the ladder's bottom rung on their own signed numbers; two fit no rung; tuppence and ludlow publish no size and no obligations. Options: (a) props: tune size, appetite and the reduction table so a band visibly crosses, and say so on the deck; (b) plausible firms: then a retailer with a £40,000 appetite against £86m turnover is not plausible, and the calibration is a real modelling job. Folded in: is a fourth adopter a goal, and are the three adopters one shared pinned package or three independent forks?

➡️ My call: (a), said out loud, with appetite expressed as a share of turnover so the ladder has room.

❓ **Q8** - **What is the truth surface for, and what would green mean?** You said "it's not good till it's green". No line has ever had fail=0. Twelve scripts can never exit 0 on the runner because the clock creates no cluster; the ceiling is about 70 of 84 and nothing states it. Options: (a) green means fail 0 and every skip owned: then the clock gets an ephemeral cluster; (b) green means the offline half plus the adopters' sampled facts, with the ceiling published and the cage graded through two more lane facts; (c) a private regression alarm, not citable evidence, and the deck stops quoting it.

➡️ My call: (b) plus publish the ceiling. You already accepted the ephemeral lane in re-grill 1.

❓ **Q9** - **Is the talk the driver or the byproduct?** Twice on 2026-07-23 you said work backwards from the talk. My twin map wrote "the talk is a byproduct" and that line was carried into NORTH-STAR §6 attributed to my document. Options: (a) driver: the stale deck, the three unfired beats and the runbook's untrue reconcile line become top priority; (b) byproduct: they drop below a dozen other things.

➡️ My call: (a). It is your instruction; the reversal is mine.

❓ **Q10** - **Must the twin forecast, or is scoring a recorded belief the thing?** Every probability the twin scores is a number read from YAML, and the package prints that fact at run time. NORTH-STAR §2 says "priced forecasts, scored against reality". Options: (a) derive a probability from signals, which reopens the ordinal-arithmetic and grade-5 rulings and needs a model call inside a reviewed-PR skill; (b) restate the twin row and beat 5 to "a recorded belief, honestly scored under proper scoring rules", require an evidence grade and basis on every world model, and keep building the scoring apparatus.

➡️ My call: (b), with the row restated. Folded in: may the forward-intel feed stay outside every signed tag?

❓ **Q11** - **How do you want to be asked, and is a bare "Agree" a ratification?** About 84 architectural items are provisional on tickets marked resolved, against the map's own rule. The batch shape was your instruction. The one-page panel-verdict shape is the only format that has drawn a reasoned reply. Options: (a) a bare agree does not ratify: seventeen tickets reopen and the items are re-put in about seventeen panel rounds; (b) the recommendations are the architecture, recorded as the assistant's, and the vocabulary retires.

➡️ My call: (b), explicitly recorded, with this round as the exception list. Folded in: when a build discovers a fact mid-run that forces an architectural call, interrupt you or decide and record?

❓ **Q12** - **Is the identity and attestation substrate spine or shelf?** Artefact attestation is real: 24 of 24 tags verify against Rekor. Actor attestation has never been observed on any citable run; all six identity scripts skip; federation is one trust domain with no peer. Ticket 12 recorded "spine, not cut" on a bare agree. Options: (a) spine: build an identity lane so the actor half is graded; (b) designed and shelved for this build: §1 reads "every artefact is attestable", the six scripts move to the exclusions file with a reason, and the claim becomes true.

➡️ My call: (b) for this build, (a) as the next thing. Claiming a thing while never observing it is the failure the Docker post-mortem exists to prevent.
