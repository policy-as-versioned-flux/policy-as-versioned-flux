...
- **Repair, 2026-09-04** — the 2026-09-03 build caused eight regressions, all from a change landing in a unit repository without its counterpart elsewhere: ticket 78's removal of the proposal `signed` key broke the hub's provenance chain and platform's wardley check, and its tighten-only clamp made step 3 demand a pull request the proposer must now refuse; ticket 63's second declared version killed three declared skip reasons; ticket 72's new script declared none. Step 3 now grades BOTH tighten-only outcomes, the hold against the real declaration and the land against a copy it loosens itself. A latent defect was fixed with them: the gate trimmed each exclusion reason with bare `xargs`, so an apostrophe truncated two reasons on the runner and dropped them here. The record is [REPAIR-2026-09-04.md](REPAIR-2026-09-04.md), which decomposes the local `fail=24` row by row as 8 regressions, 11 substrate artefacts and 5 estate-state reds.
