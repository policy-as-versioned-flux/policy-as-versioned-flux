...
`tests/test_untagged_pin.py` loads `untagged_pin.py`, which imports it in turn.
`.github/workflows/twin.yml`'s `tests` job never installed it.

**Corrected 2026-09-04 after review.** An earlier draft of this section said ticket 69 added the
`jsonschema` import. It did not. `git log -S jsonschema -- verify/feed-contract/feed_contract.py`
puts the import at `3e83a16`, 2026-08-28, ticket 21. What changed on 2026-09-04 is that tests
began to REACH it, and two tickets did that, not one. Run 33841231240 (`73325aa6`, 05:38Z, before
PR 18) reports zero errors. Run 33851989099 (`7c019a8`, ticket 69) reports 24. Run 33862568750
(`3d1a348`, ticket 76) reports 25, the extra one through `tests/test_every_green.py`. The shape
was also overstated: one of the 25 is a collection error, in `test_every_green.py`; the other 24
are setup errors in `test_untagged_pin.py`.
...
One worktree, one variable, with `tests/test_enact.py` AS IT STANDS ON `main` (`13234b5`).
`.estate-clone` absent throughout, which is the harsher of the two runner conditions. With this
branch's test file every row reads 61 passed, because the fixture arms the guard and the mode is
...
## Confirmed on the runner, not only here

Run 33912974398, the push of `0c9e6d4` to this branch — the same machine class that produced the
red. `0c9e6d4` is not the branch tip; the commits after it are documentation. Run 33913449656
(`6194d85`) reports the same numbers: `tests` 1 failed / 1764 passed, `invariants` 71 passed /
1 failed / 3 skipped, and `typecheck`, `demo`, the three determinism jobs and
`reproduce-elsewhere` all green.

| job | before (run 33893872728, `main`) | after |
| --- | --- | --- |
| `tests` | failure — 36 failed, 1682 passed, 25 errors | failure — **1 failed, 1764 passed** in 128.81s |
| `typecheck` | failure — 1 error | **success** |
| `invariants` | failure — 71 passed, 1 failed, 3 skipped | failure — **71 passed, 1 failed, 3 skipped** (unchanged) |
| `demo` | — | success |
| `determinism` ×3, `reproduce-elsewhere` | — | success |

The runner's numbers are the local ones exactly. Both remaining reds are the **same single
invariant**, item 4, reported twice: once by the `invariants` job and once by
`tests/test_invariant_suite.py::test_the_suite_is_green`, which mirrors it —

```
45  FAIL  flux_coverage_floor_is_still_reachable   the pre-registered coverage floor of 90% can no
longer be reached: 3/1966 sample(s), only 62 days, 4:11:34 of window left ... a ceiling of 68.5%.
**This guard staying red is the finding, not a defect in it**
RESULT: 71 passed, 1 failed, 3 skipped (0 pending invariants, 3 skipped and not faked)
```

Worth stating plainly for whoever next asks why the workflow is not all green: **`tests` and
`invariants` cannot both go green until invariant 45 is resolved**, because `test_the_suite_is_green`
asserts the whole suite. That was true before this branch and is unchanged by it. Nothing else in
either job is red.

## Review round 2: the estate had lost its only alarm on the switch

The reviewer accepted the repair and blocked on one thing, correctly. Arming the fixture was
right, and the reviewer proved it rather than granting it: with `main`'s arrangement, mutating
`decide()` to admit everything moves the count from 35 failures to 40, so a catastrophically
broken guard was a five-test difference inside a wall of red. With the fixture armed the same
mutation moves it from 0 to 39, and a narrower loosening of the `other-hand` branch alone moves
it from 0 to 6. The armed suite is a stronger capability test, not a weaker one.

What the reviewer would not accept is what replaced the alarm. The old test went red when the
mode moved. The new one passes under every mode, so an accidental or malicious flip to
`development` turned nothing red anywhere. The reviewer then searched for any other signal and
found none: `decide()` prints only when it denies, so a permissive mode is silent by
construction; invariant 48 forces `operations` and never named the ambient mode; `talk/
verify-all.sh` has no occurrence of it and the TRUTH line carries no mode field;
`talk/local-clock.sh` and its test grade the script's own text. After the branch, the mode was
observable only by opening the file.

**Fixed here (delegated, ADR-0025).** Invariant 48 now reads the AMBIENT mode before it forces
`operations`, and reports it on its detail line, with a clause naming the cost when the mode is
`development`. It is reported and not graded: which mode the estate runs in is the owner's
authorisation, and ADR-0025 keeps authorisations with the owner. So `./bin/twin verify` and the
`invariants` job now say, on the line they already print:

    This check FORCED `operations` to test the capability; the estate is running at
    `development`, which admits every merge and every enactment push

Proved to track the real state rather than a constant: the same command under `operations` and
`other-hand` prints those words instead.

**Not done here, and named rather than quietly dropped.** The reviewer's second suggestion was to
carry `enact=<mode>` on the TRUTH line as well. Twelve files parse that line, and two of
`verify-all.sh`'s own selfcheck patterns are anchored with `$`, so a new field there is a pass
across the whole record surface and not a two-line change. It is its own ticket. The third
suggestion, an acknowledgement file pairing the mode with the commit that authorised it so an
UNRECORDED flip goes red, is the better long-term answer and is also its own ticket: it invents
an authorisation shape, and ADR-0025 keeps those with the owner.
...
- `twin/invariants/harness.py` — invariant 48 reports the ambient enact mode (review round 2).
...
the enactment-push refusal is off. The suite now says so in a passing test, and `./bin/twin
verify` says so out loud on invariant 48's line, instead of a red wall. Closing the window is one
word — write `other-hand` into `twin/ENACT_MODE` — and both follow it without further edits.

Two follow-up tickets fall out of the round-2 review and are charted, not built:

1. Carry `enact=<mode>` on the TRUTH line, so the estate's one citable line says whether the twin
   may write to the world. Twelve files parse that line; this is a pass across all of them.
2. Pair `twin/ENACT_MODE` with a companion record naming the commit that authorised the mode, so
   a flip nobody recorded goes red while a recorded one like `f959187` stays green. This invents
   an authorisation shape, which ADR-0025 keeps with the owner.
