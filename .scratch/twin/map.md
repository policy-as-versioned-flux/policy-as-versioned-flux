...
- **Backtest roster gap: opportunity cases.** Every chosen backtest case (Carillion, Enron, Wirecard, NMC, Kodak) is a **collapse** — the suite validates an engine promising *fear AND opportunity* exclusively on catastrophes. Needs **seized-opportunity** and **missed-opportunity** cases with dated contemporaneous evidence. Raised in ticket 12; owned by the backtest workstream.
- **The enumerated sensor set + the DPIA workflow** — the *rules* are decided (ticket 15); the concrete
  sensor list and the operational DPIA/gate workflow are build-time artefacts still to be produced.
